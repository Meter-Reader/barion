...
module Barion
  ...
  class PaymentTest < ActiveSupport::TestCase
    ...
    test 'payment type sets reservation period' do
      ...
      assert_predicate @payment, :reservation?
      ...
      refute_predicate @payment, :reservation?
      assert_nil @payment.reservation_period
    ...
    test 'payment request id unique' do
      ...
      1000.times do
        ...
        refute_includes ids, obj.payment_request_id, msg: obj.payment_request_id
    ...
    test 'transactions has at least one element' do
      assert_predicate @payment.payment_transactions.size, :positive?
    ...
    test 'payment is readonly if not in initalization status' do
      assert_valid @payment
      assert_predicate @payment, :initial?
      refute_predicate @payment, :readonly?
      @payment.status = :prepared

      refute_predicate @payment, :initial?
      assert_predicate @payment, :readonly?
    ...
    test 'checksum is checked when loading from db' do
      ...
      assert_raises ::Barion::TamperedData do
        ::Barion::Payment.find(@payment.id) # data change detected, raise exception
      end
    end
